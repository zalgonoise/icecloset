...
func UnmarshalObject(ctx context.Context, obj *PerfObject, vs interface{}, logger *slog.Logger) error {
	if obj == nil {
	...
	for idx, instance := range obj.Instances {
		...
		for i := range target.NumField() {
			...
			if !found {
				logger.DebugContext(ctx, fmt.Sprintf("missing counter %q, have %v", tag, counterMapKeys(counters)))
				continue
			}
